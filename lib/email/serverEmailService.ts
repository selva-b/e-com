...
import { EmailTemplateType } from './emailService';

// Interface for server-side email data
...
// Send email function for server-side use
export const sendServerEmail = async (emailData: ServerEmailData) => {
  try {
    // Create transporter
    ...
    const subject = emailData.subject || replaceTemplateVariables(template.subject, emailData.variables);
